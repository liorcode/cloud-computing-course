...
The API is a REST API that allows users to create users, groups, send messages to users and groups, and get messages sent to a user.
...
The group message processor lambda will read from the SQS queue and send the message to each member of the group.
...
- Our "create" users/groups/direct message lambdas are simple and should very fast. All they do is write to the database and return the generated id.
